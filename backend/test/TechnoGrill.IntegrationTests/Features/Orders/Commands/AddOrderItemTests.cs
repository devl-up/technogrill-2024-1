...
public sealed class AddOrderItemTests(DatabaseFixture databaseFixture)
{
    [Fact]
    public async Task AddOrderItem_Should_ReturnNoContent_When_Successful()
    {
        // Arrange
        ...
        var command = new AddOrderItem.Command(orderId, product.Id, 10);

        // Act
        var response = await client.PostAsJsonAsync("api/order/add-item", command);
