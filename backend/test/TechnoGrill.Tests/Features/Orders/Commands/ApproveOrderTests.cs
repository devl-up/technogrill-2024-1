...
public sealed class ApproveOrderTests
{
    private readonly ApproveOrder.Validator _sut = new();

    [Fact]
    public async Task Validate_Should_HaveNoErrors_When_Valid()
    {
        // Arrange
        var command = new ApproveOrder.Command(Guid.NewGuid());
    ...
    {
        // Arrange
        var command = new ApproveOrder.Command(Guid.Empty);
