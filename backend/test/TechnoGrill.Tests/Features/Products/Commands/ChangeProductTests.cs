...
public sealed class ChangeProductTests
{
    private readonly ChangeProduct.Validator _sut = new();

    [Fact]
    public async Task Validate_Should_HaveNoErrors_When_Valid()
    {
        // Arrange
        var command = new ChangeProduct.Command(Guid.NewGuid(), "name", "description", 10);
    ...
    {
        // Arrange
        var command = new ChangeProduct.Command(id, name, description, amount);
